...
class ClientEnquiry < ActionMailer::Base
  def currency(data, emsets, sent_at = Time.now)
    ...
    #bcc        '1st@virtual-presence.co.uk'
  ...
  def mortgage1(data, emsets, sent_at = Time.now)
   ...
   # bcc        [data[:magent].email1, data[:magent].email2]
    bcc        [data[:magent].email1, data[:magent].email2, '1st@virtual-presence.co.uk']
  ...
  def privateClient(data, emsets, sent_at = Time.now)
    ...
    if data[:defagent]
    #  bcc     [data[:defagent].email1, '1st@virtual-presence.co.uk']
			bcc     [data[:defagent].email1]
    else
    #  bcc     '1st@virtual-presence.co.uk'
    end
  ...
  def noagent(data, sent_at = Time.now)
    ...
    recipients ["Louisa@1st-for-french-property.co.uk", "fiona@virtual-presence.co.uk" ]
