...
class ProcessOldEmails
  ...
  def run(from, to) 
    ...
    # TODO include ignored and done check
    recs = OldEmail.find(:all, 
  ...
  def parse_one_email(rec)
    ...
    if enq == nil
      ...
      agents.each {|ag|
        rem = ag.agent_reminders.find_by_closed_at(nil)
        AgentEnquiry.create(
          :enquiry_id => enq.id(),
          :agent_id => ag.id(),
          :agent_reminder_id => rem.id(),
          :sent_at => rec.sent_at,
          :created_at => rec.stored_at
        )
        @matched += 1
