# Create the six monthly reminders and link to agent_enquiries
...
class SixMonthlyRemindersCreator
  ...
  def run
    ...
    agents = Agent.find_all_by_get_6m_rem(true)
    agents.each {|ag|
      ag_rem_count = 0
      name = ag.firstname
      ...
      ar = ag.agent_reminders.find_by_month_and_rem_type_and_sent(@month, 'six-monthly', false)

      # if not found create one
      if ar == nil
        ar = ag.agent_reminders.create(
          :month => @month,
          :rem_type => 'six-monthly'
        )
        @new_ar_cnt += 1
      else
        @exist_ar_cnt += 1
      ...
      ag_enqs = AgentEnquiry.find_all_by_agent_id_and_rem_6m_sent_and_ignore_it(ag.id, false, false)
      #puts "Found #{ag_enqs.size} enquiries for #{ag}" if ag_enqs.size > 0
      ag_enqs.each {|ae|
        if ae.sent_at >= @startdate and ae.sent_at <= @enddate
          ae.agent_reminder_6m_id = ar.id()
          ae.save
          @ae_cnt += 1
          ag_rem_count += 1
        else
