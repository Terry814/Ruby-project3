...
class EnquiryHandler
  ...
  # get the default setting for emails
  ...
  # get the special settings for this agent if any
  def get_bargain_email_settings
    sets =  AgentRemindersSetting.find_by_rem_type_and_agent_id("Bargain Client", @agent.id)
    if sets
    ...
    sets = AgentRemindersSetting.find_by_rem_type_and_agent_id("Bargain Agent", @agent.id)
    @bargain_agent_settings = sets if sets
  end
